...
class ICPNode(Node):
    def __init__(self):
        ...
        self.pcd_sub = self.create_subscription(PointCloud2, "/velodyne_points", self.pcd_callback, 10)# TODO: Task 3.3
        self.marker_pub = self.create_publisher(Marker, "/marker", 10)# TODO: Task 3.4
    ...
    def pcd_callback(self, msg: PointCloud2):
        ...
        transformed_points = (self.lidar_pose @ points_homogeneous.T).T
        ...
        if self.use_open3d:
            result = open3d_icp(source, target, initial_guess)
            self.transformation = result.transformation
        else:
            source_np = np.asarray(source.points)
            target_np = np.asarray(target.points)
            self.transformation = icp(source_np, target_np, init_pose=initial_guess) 
        ### Task 3.6 ###
