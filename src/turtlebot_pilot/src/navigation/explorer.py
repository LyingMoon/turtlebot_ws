...
def explore(occupancy: StochOccupancyGrid2D):
    ...
    # For counting -1.0 Element, 1 means unknown
    ...
    # For counting 0.5<= <=1.0 element. 1 means known occupied
    ...
    # For counting 0.0<= <0.5 element. 1 means known unoccupied
...
class Explorer(Node):
    def __init__(self):
        ...
        self.ignore_stop_sign_duration = 20.0  # Ignore stop sign duration after resuming
        self.stop_resume_time = None  # Time to resume after stopping
    ...
    def start_exploration(self):
        ...
        if not self.active:
            ...
            self.get_logger().info(f"    CURR TIME: {current_time}")
            return

        
        
        if not self.is_exploring or self.is_finish:
            self.get_logger().warn("Exploration Stopped...")
            return
    ...
    def detector_callback(self, msg: Bool):
        ...
        # Ignore the stop sign if within the ignore duration
        if self.ignore_until_time and self.stop_resume_time and \
            current_time < self.ignore_until_time and current_time >= self.stop_resume_time:
            self.get_logger().info("    RESUME IN DETECTOR...")
            self.stop_resume_time = None
        ...
        if msg.data:
            ...
            self.publish_goal(x=self.current_state.x, y=self.current_state.y, theta = self.current_state.theta)
            self.ignore_until_time = current_time + self.ignore_stop_sign_duration
            self.stop_resume_time = current_time + self.stop_duration
            self.get_logger().info(f"    SET RESUME TIME:{self.stop_resume_time}")
            self.get_logger().info(f"    SET IGNORE TIME:{self.ignore_until_time}")
